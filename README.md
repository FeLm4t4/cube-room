...
手動で面を回すと、その操作を反映した解法に更新します。解法はシャッフルの逆手順をもとに、同じ軸の回転の合成や、同じ変化を起こす短い手順への置き換えで短縮します。**最短手数を保証するものではありません。** 区間の短縮はWeb Workerで行います。再生中は手順を固定し、停止すると残りの手順を自動更新します。たどった手順と現在位置はそのまま残ります。
