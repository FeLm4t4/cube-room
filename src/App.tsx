...
import {
    CubeStage,
    type CubePerformance,
    type CubeStageHandle,
} from "./components/CubeStage";
...
const SPEEDS = [
    ...
    { label: "標準", duration: 220 },
    { label: "速い", duration: 120 },
];
...
export default function App() {
    ...
    const [interactionMode, setInteractionMode] = useState<"turn" | "orbit">(
        "turn",
    );
    const [showFps, setShowFps] = useState(false);
    const [performanceStats, setPerformanceStats] =
        useState<CubePerformance | null>(null);
    const [modal, setModal] = useState<ModalKind>(null);
    const modalRef = useRef<ModalKind>(null);
    const dialogRef = useRef<HTMLDialogElement>(null);
    const returnFocusRef = useRef<HTMLElement | null>(null);
    const [error, setError] = useState("");
    const fullscreen = useFullscreen(setError);
    ...
    const pendingCursorRef = useRef<{ move: Move; cursor: number } | null>(
        null,
    );
    ...
    const {
        moves: solutionMoves,
        movesRef: solutionMovesRef,
        optimizing,
        apply: applySolution,
        reset: resetSolution,
        interrupt: interruptSolution,
        setInteractionActive,
    } = useSolution(ready && !busy, canAcceptSolution, getCubeForSolution, searchSeconds);
    const changeSearchSeconds = useCallback((seconds: 1 | 5 | 10) => {
        setSearchSeconds(seconds);
        try {
            localStorage.setItem("cube-room.searchSeconds", String(seconds));
        } catch {
            // 保存できない環境でも、開いている間は選択した上限を使う。
        }
    }, []);
    const rebaseTrail = useCallback((moves: readonly Move[]) => {
        ...
        if (
            previous.cursor === 0 &&
            previous.moves.length === moves.length &&
            moves.every((move, index) => move === previous.moves[index])
        )
            return;
    ...
    useEffect(() => {
        ...
        if (
            remaining.length === currentSolution.length &&
            remaining.every((move, index) => {
                const next = currentSolution[index];
                return (
                    move.axis === next.axis &&
                    move.layer === next.layer &&
                    move.turns === next.turns
                );
            })
        )
            return;
        ...
        const next = {
            moves: [
                ...previous.moves.slice(0, previous.cursor),
                ...currentSolution,
            ],
    ...
    const onMove = useCallback(
        (move: Move, source: "user" | "program") => {
            const nowSolved = stageRef.current
                ? isSolved(stageRef.current.getCube())
                : false;
            const nextSolution = applySolution(move, nowSolved);
            const pending = pendingCursorRef.current;
            if (
                operationRef.current === "solution" &&
                source === "program" &&
                pending &&
                pending.move.axis === move.axis &&
                pending.move.layer === move.layer &&
                pending.move.turns === move.turns
            ) {
            ...
            setSolved(nowSolved);
            if (
                operationRef.current === "shuffle" ||
                operationRef.current === "undo"
            )
                return;
    ...
    const playTurn = useCallback(
        async (move: Move, reverse = false) => {
            if (!beginOperation("play")) return;
            const generation = generationRef.current;
            try {
                await stageRef.current!.playMove(
                    reverse ? inverseMove(move) : move,
                    speedRef.current,
                );
            } catch {
                if (generation === generationRef.current)
                    setError(
                        "回転を完了できませんでした。もう一度お試しください。",
                    );
            } finally {
    ...
    const undo = useCallback(async () => {
        ...
        try {
            ...
            if (generation === generationRef.current)
                setError("一手戻せませんでした。もう一度お試しください。");
        } finally {
            if (generation === generationRef.current) endOperation();
        }
    ...
    const shuffle = useCallback(async () => {
        ...
        try {
            ...
            const length =
                ...
                Math.floor(
                    Math.random() *
                        (MAX_SHUFFLE_LENGTH - MIN_SHUFFLE_LENGTH + 1),
                );
            const moves = createScramble(length);
            setShuffleLength(length);
            for (let index = 0; index < moves.length; index += 1) {
                if (generation !== generationRef.current || !stageRef.current)
                    return;
                await stageRef.current!.playMove(
                    moves[index],
                    Math.min(speedRef.current, 50),
                );
                if (generation !== generationRef.current) return;
    ...
    const seekSolution = useCallback(
        async (target: number) => {
            const path = trailRef.current.moves;
            if (
                !solutionVisibleRef.current ||
                target === trailRef.current.cursor ||
                target < 0 ||
                target > path.length
            )
                return;
            ...
            try {
                while (
                    trailRef.current.cursor !== target &&
    ...
    const requestReset = useCallback(() => {
        if (historyRef.current.length || scrambledRef.current)
            openModal("reset");
        else reset();
    }, [openModal, reset]);

    const actionsRef = useRef({ playTurn, undo, shuffle, openModal });
    actionsRef.current = { playTurn, undo, shuffle, openModal };
        ...
        const onKeyDown = (event: KeyboardEvent) => {
            ...
            if (
                ...
                (target.isContentEditable ||
                    target.closest('input:not([type="checkbox"]):not([type="radio"]), textarea, select'))
            )
                return;
            if (
            ...
            if (event.code === "Space") {
                if (
                    target instanceof HTMLElement &&
                    target.closest('button, input, a, summary, [role="button"]')
                )
                    return;
                event.preventDefault();
                ...
                event.preventDefault();
                void actionsRef.current.playTurn(
                    control.move,
                    event.shiftKey || inverseRef.current,
                );
            } else if (event.key === "?") {
                event.preventDefault();
    ...
    const status = !ready
        ? "キューブを準備しています"
        : shuffling
          ? `${shuffleLength}手でシャッフルしています`
          : solutionPlaying
            ? "解法例をたどっています"
            : won
              ? "6面が揃いました"
              : scrambled || history.length
                ? "プレイ中"
                : "完成状態";
    const renderModeLabel =
        performanceStats?.renderMode === "GPU"
            ? "GPU / WebGL 2"
            : performanceStats?.renderMode === "software"
              ? "ソフトウェア / WebGL 2"
              : "WebGL 2";

    return (
        <div className="app-shell" ref={fullscreen.ref}>
            <a href="#play" className="skip-link">
                キューブの操作へ進む
            </a>
            <header className="site-header">
                <a className="brand" href="#play" aria-label="CUBE ROOM ホーム">
                    <span className="brand-mark">
                        <Icon name="cube" size={30} strokeWidth={1.5} />
                    </span>
                    <span>
                        CUBE ROOM<span className="brand-period">.</span>
                    </span>
                </a>
                <nav className="header-nav" aria-label="メインナビゲーション">
                    ...
                    >
                        GitHub <Icon name="arrow-up-right" size={15} />
                    </a>
                </nav>
            </header>
                    ...
                    >
                        <div className="stage-topbar">
                            ...
                            <div className="stage-actions">
                                <button
                                    type="button"
                                    className={`fps-toggle${showFps ? " selected" : ""}`}
                                    aria-label="FPS表示"
                                    aria-pressed={showFps}
                                    onClick={() => {
                                        if (!showFps) setPerformanceStats(null);
                                        setShowFps(!showFps);
                                    }}
                                >
                                    FPS
                                ...
                                <button
                                    ...
                                    title={!fullscreen.supported ? "このブラウザでは全画面表示を利用できません" : fullscreen.isFullscreen ? "全画面表示を解除（Esc）" : "全画面表示"}
                                >
                                    <Icon name={fullscreen.isFullscreen ? "minimize" : "maximize"} size={17} />
                                    <span>{fullscreen.isFullscreen ? "解除" : "全画面"}</span>
                                </button>
                                <button
                                    type="button"
                                    className="icon-button view-reset"
                                    onClick={() =>
                                        stageRef.current?.resetView()
                                    }
                                    disabled={!ready}
                                    aria-label="キューブの視点を元に戻す"
                                    title="視点を元に戻す"
                                >
                        ...
                        <div className="cube-stage-slot">
                            <CubeStage
                                ...
                                onError={setError}
                                showFps={showFps}
                                onPerformance={setPerformanceStats}
                            />
                        </div>
                        {won && (
                        ...
                        <div className="stage-bottom">
                            <div className="stage-drag-hints">
                                <span>
                                    <span className="mouse-hint mouse-hint-left" />{" "}
                                    左ドラッグで回転
                                </span>
                                <span>
                                    <span className="mouse-hint mouse-hint-right" />{" "}
                                    右ドラッグで視点移動
                                </span>
                            </div>
                            <div
                            ...
                            >
                                <button
                                    ...
                                    aria-pressed={interactionMode === "turn"}
                                    onClick={() => setInteractionMode("turn")}
                                >
                                    <Icon name="turn" size={16} /> 回す
                                </button>
                                <button
                                    type="button"
                                    className={
                    ...
                    >
                        <div className="panel-heading">
                            ...
                            <button
                                ...
                                aria-label="キューブと記録をリセット"
                            >
                                <Icon name="reset" size={14} /> リセット
                            </button>
                        </div>
                        <div className="session-stats">
                            <div>
                                ...
                                <span
                                    className="stat-value timer-value"
                                    aria-label={`経過時間 ${formatTime(elapsed)}`}
                                >
                                    {formatTime(elapsed)}
                                    <span
                                        className={`timer-dot${timerRunning ? " is-running" : ""}`}
                                    />
                                </span>
                            </div>
                            <div>
                                <span className="stat-label">手数</span>
                                <span className="stat-value move-value">
                                    {String(history.length).padStart(2, "0")}
                                    <span className="stat-unit">手</span>
                                </span>
                            </div>
                        </div>
                        <p
                            className={`session-status${won ? " status-solved" : ""}`}
                            aria-live="polite"
                        >
                            {won ? (
                                ...
                                <span
                                    className={`status-dot${shuffling ? " is-shuffling" : ""}`}
                                />
                            )}
                            {status}
                        </p>
                        ...
                        >
                            ...
                            <span>
                                {shuffling
                                    ? `シャッフル中 ${shuffleProgress}/${shuffleLength}`
                                    : "シャッフル"}
                            </span>
                            <span className="shuffle-length">
                                {shuffleLength ??
                                    `${MIN_SHUFFLE_LENGTH}〜${MAX_SHUFFLE_LENGTH}`}
                                手
                            </span>
                        </button>
                        <button
                            ...
                            onClick={() => void undo()}
                            disabled={controlsDisabled || !history.length}
                        >
                        ...
                        <button
                            ...
                            aria-controls="solution-trail"
                        >
                            <Icon name="spark" size={17} />
                            <span>解法例</span>
                            <span className="solution-count">
                                {solutionMoves.length
                                    ? `残り ${solutionMoves.length}手`
                                    : "完成"}
                            </span>
                        </button>
                        ...
                        <label className="inverse-toggle">
                            <input
                                ...
                                checked={inverse}
                                onChange={(event) => {
                                    inverseRef.current = event.target.checked;
                                    setInverse(event.target.checked);
                                }}
                            />
                            ...
                            >
                                {SPEEDS.map((option) => (
                                    <button
                                        type="button"
                                        key={option.duration}
                                        aria-pressed={speed === option.duration}
                                        className={
                                        ...
                                        onClick={() => {
                                            speedRef.current = option.duration;
                                            setSpeed(option.duration);
                                        }}
                                    >
                                        {option.label}
                                    </button>
                            ...
                            {history.length ? (
                                <>
                                    ...
                                    {history.slice(-10).map((move, index) => (
                                        <span
                                            className={`move-chip${index === Math.min(history.length, 10) - 1 ? " latest" : ""}`}
                                            key={`${history.length - Math.min(history.length, 10) + index}-${moveToNotation(move)}`}
                                        >
                                            {moveToNotation(move)}
                                        </span>
                                    ))}
            ...
            <dialog
                ref={dialogRef}
                className={`app-dialog ${modal === "reset" ? "reset-dialog" : "help-dialog"}`}
            ...
            >
                <div className="dialog-content">
                    ...
                    {modal === "reset" ? (
                        ...
                        <>
                            <h2 id="dialog-title">操作ガイド</h2>
                            <p className="dialog-description">
                                シャッフルしたキューブの6つの面を、それぞれ同じ色に揃えたら完成です。
                            </p>
                            <div className="help-gestures">
                                <div>
                                    <span className="help-icon">
                                        <Icon name="turn" size={24} />
                                    </span>
                                    <h3>面を回す</h3>
                                    <p>
                                        色のついた面を左ドラッグ。中央の段・列も、動かした方向に回転します。
                                    </p>
                                </div>
                                <div>
                                    <span className="help-icon">
                                    ...
                                    <h3>視点を変える</h3>
                                    <p>
                                        右ドラッグで本体の向きを変えます。ホイールで拡大・縮小できます。
                                    </p>
                                </div>
                            </div>
                            <p className="touch-help">
                                <strong>スマートフォン・タブレット</strong>
                                <span>
                                    「回す」で面をスワイプ、「見る」で視点を移動。2本指のピンチで拡大・縮小できます。
                                </span>
                            </p>
                            <div className="keyboard-help">
                                ...
                                <div className="shortcut-list">
                                    <p>
                                        ...
                                        <span>上・手前・右の面を回す</span>
                                    </p>
                                    <p>
                                        <span>
                                            <kbd>A</kbd><kbd>S</kbd><kbd>D</kbd>
                                        </span>
                                        <span>下・奥・左の面を回す</span>
                                    </p>
                                    <p>
                                        <span>
                                            <kbd>Z</kbd><kbd>X</kbd><kbd>C</kbd>
                                        </span>
                                        <span>左右・上下・前後の間の層を回す</span>
                                    </p>
                                    <p>
                                        <span>
                                            <kbd>Shift</kbd>
                            ...
                            <p className="notation-help">
                                プレビュー右上の「全画面」で、操作パネルも一緒に広がります。「解除」か Esc キーで元に戻ります。対応ブラウザで利用できます。
                            </p>
                            <button
                                type="button"
                                className="primary-button help-start"
                                onClick={closeModal}
                            >
                                閉じる
